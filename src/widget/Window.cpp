...
static gboolean _on_quit (GtkWidget *widget, GdkEvent  *event, gpointer  p) {
	Window* w = (Window*)p;
	w->on_quit();
	return TRUE;
}
...
Window::Window() : Widget(NULL){
	...
	g_signal_connect(widget, "delete-event", G_CALLBACK(_on_quit), this);
...
void Window::on_quit() {
	...
	if(bQuit) { DBG("BYE BYE"); gtk_main_quit(); }
}
